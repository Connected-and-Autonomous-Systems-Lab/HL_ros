...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    with conn:
        ...
        while (enable):
            ...
            print(Fore.WHITE + 'Pose at time {ts}'.format(ts=data.timestamp))
            print(data.pose)
            imu_data = hl2ss.unpack_rm_imu(data.payload)
            count = imu_data.get_count()
            sample = imu_data.get_frame(0)
            print(Fore.WHITE + 'Got {count} samples at time {ts}, first sample is (ticks = {st}, x = {x}, y = {y}, z = {z})'.format(count=count, ts=data.timestamp, st=sample.sensor_ticks_ns, x=sample.x, y=sample.y, z=sample.z))
            
            ts_converted = struct.pack("!Q", data.timestamp)
            conn.sendall(ts_converted)
